"""Valida los notebooks del curso de LangSmith sin ejecutarlos (no hace falta clave).
...
OPCIONALES = {
    # Del grupo `pruebas-con-modelo`, que no se instala por defecto (notebook 10).
    "vcr", "openai",
    # Del curso de LangGraph, que este importa para instrumentarlo.
    "langgraph_sdk", "langgraph.checkpoint.postgres", "psycopg",
    # De presentación y de desarrollo.
    "matplotlib", "matplotlib.pyplot", "IPython", "IPython.display", "pytest", "dotenv",
}

#: `langsmith` NO está en la lista de arriba, y es deliberado: comprobar que cada
#: símbolo del SDK existe es el filtro que sostiene la honestidad de este curso.
