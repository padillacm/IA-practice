"""Ejecuta los notebooks del curso de LangSmith de principio a fin, sin servicio.
...
Y hace una cosa más que su hermano del curso de LangGraph, que aquí es el punto
entero: **corta la salida a `api.smith.langchain.com`**. Los notebooks de este curso
están escritos en dos modos —sin clave se ejecutan enteros en local, con clave se
conectan—, y la única forma de comprobar que el modo local es de verdad local es
impedir la conexión y ver que aun así terminan. Si un notebook intenta salir a la
red durante esta ejecución, falla aquí y no en tu cuota.

El corte se hace en `modelo_falso.aislar_langsmith()`, que además pone el entorno en
el estado «sin clave» para que `init()` tome esa rama.

Uso:
    uv run _tools/ejecutar_notebooks.py               # todos
    uv run _tools/ejecutar_notebooks.py 01_trazas/    # solo una carpeta
    uv run _tools/ejecutar_notebooks.py 01_trazas/01_anatomia_de_una_traza.ipynb
